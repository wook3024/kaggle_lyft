...
class LyftMultiModel_image(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        try:
            ...
            pass
        try:
            os.mkdir(f'./result/train/{self.model_name}')
        except:
            pass
        try:
            os.mkdir(f'./result/train/{self.model_name}/results')
        except:
            pass
        try:
            os.mkdir(f'./result/train/{self.model_name}/models')
        except:
            pass
        try:
            os.mkdir(f'./result/train/{self.model_name}/logs')
        except:
            print(f'{self.model_name} folder is already exgist!')
        ...
        self.backbone_linear = nn.Sequential(
            nn.Linear(in_features=2048, out_features=2024)
        )

        self.conv1d_head = nn.Sequential(
            nn.Linear(in_features=11*60, out_features=4096),
    ...
    def forward(self, images, history_yaws, history_positions):
        ...
        # print(x.shape)
        # features = x.view(x.shape[0], 16, 128)
        features = x.view(x.shape[0], 32, 64)
        # print(features.shape)
        features = self.conv1d_family1(features)
        features = self.conv1d_family2(features)
        # features = self.conv1d_family2(features)
        # features = self.conv1d_family2(features)
        # print(features.shape)
        x = torch.flatten(features, 1)
        # features = self.backbone_linear(x)
        # features = features.view(features.shape[0], 11, -1)
        # print(x.shape)            
        x = self.conv1d_head(x)
        
        preds = self.x_preds(x)
        modes = self.x_modes(x)
        return preds, modes, features



class LyftMultiModel_final(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.parent_head = nn.Sequential(
            nn.Linear(in_features=11*128, out_features=4096),
        )
        ...
        kernsel_size=3
        self.feature_preds = nn.Sequential(
            nn.Linear(in_features=11*59, out_features=self.num_preds)
        )
        self.feature_modes = nn.Sequential(
            nn.Linear(in_features=11*59, out_features=self.num_modes)
        )
    ...
    def forward(self, images_features, history_yaws, history_positions):
        features = torch.cat((images_features, history_yaws, history_positions), dim=2)
        ...
        # features = self.parent_head(features)
        
        preds = self.feature_preds(features)
        modes = self.feature_modes(features)
        return preds, modes
...
image_model = LyftMultiModel_image()
# image_model = nn.DataParallel(image_model, output_device=0)
image_model.to(device)
# image_model.cuda()

final_model = LyftMultiModel_final()
# final_model = nn.DataParallel(final_model, output_device=0)
final_model.to(device)
# final_model.cuda()

image_optimizer = AdamP(image_model.parameters(), lr=cfg["model_params"]["lr"], betas=(0.9, 0.999), weight_decay=1e-2)
final_optimizer = AdamP(final_model.parameters(), lr=cfg["model_params"]["lr"], betas=(0.9, 0.999), weight_decay=1e-2)


# version = 'v51'
# pre_weight_path = f'/content/pytorch_lightning-models-{version}.ckpt'
# pre_checkpoint = torch.load(pre_weight_path)

# weight_path = '/content/resnest50+267+adamp+0.5+10_history_num_frames+1e-4+yaw_1_3300.pth'
# checkpoint = torch.load(weight_path)


# image_model.load_state_dict(checkpoint['image_model_state_dict'])
# image_model.load_state_dict(pre_checkpoint['state_dict'], strict=False)
# final_model.load_state_dict(checkpoint['final_model_state_dict'])
# image_optimizer.load_state_dict(checkpoint['image_optimizer_state_dict'])
# final_optimizer.load_state_dict(checkpoint['final_optimizer_state_dict'])
...
for epoch in range(1, 5):
    ...
    progress_bar = tqdm(range(loader_len))
    for i in progress_bar:
        ...
        # history_yaws = torch.mul(data["history_yaws"].to(device), 1.6)
        ...
        history_positions[:,:,1] = torch.div(history_positions[:,:,1],3.18)

        image_preds, image_modes, image_features = image_model.forward(inputs, history_yaws, history_positions)
        final_preds, final_modes = final_model.forward(image_features.detach(), history_yaws, history_positions)

        bs, _ = image_preds.shape

        image_preds = image_preds.view(bs, 3, 50, 2)
        assert image_modes.shape == (bs, 3)
        image_confidences = torch.softmax(image_modes, dim=1)

        final_preds = final_preds.view(bs, 3, 50, 2)
        assert final_modes.shape == (bs, 3)
        final_confidences = torch.softmax(final_modes, dim=1)

        image_loss = criterion(targets, image_preds, image_confidences, target_availabilities)
        final_loss = criterion(targets, final_preds, final_confidences, target_availabilities)
    ...
    #     losses_image.append(image_loss.item())
    #     losses_final.append(final_loss.item())

    #    progress_bar.set_description(f"im_loss: {round(image_loss.item(), 2)} fn_loss: {round(final_loss.item(), 2)}")
        progress_bar.set_description(f"im_loss: {round(image_loss.item(), 2)} fn_loss: {round(final_loss.item(), 2)}")      
        # if i % 10000 == 0 or i == loader_len - 1:
        # # torch.save({
        # #       'image_model_state_dict': image_model.module.state_dict(),
        # #       'final_model_state_dict': final_model.module.state_dict(),
        # #       'image_optimizer_state_dict': image_optimizer.module.state_dict(),
        # #       'final_optimizer_state_dict': final_optimizer.module.state_dict(),
        # #       }, f'{os.getcwd()}/result/train/{model_name}/models/{epoch}_{i}.pth')
        # torch.save({
        #         'image_model_state_dict': image_model.state_dict(),
        #         'final_model_state_dict': final_model.state_dict(),
        #         'image_optimizer_state_dict': image_optimizer.state_dict(),
        #         'final_optimizer_state_dict': final_optimizer.state_dict(),
        #         }, f'{os.getcwd()}/result/train/{model_name}/models/{epoch}_{i}.pth')
    ...
    test_it = iter(val_dataloader)

    losses_image = []
    losses_final = []

    progress_bar = tqdm(range(math.ceil(loader_len*0.2)))
    for i in progress_bar:
        data = next(test_it)
        inputs = data["image"].to(device)
        target_availabilities = data["target_availabilities"].to(device)
    ...
    #     progress_bar.set_description(f"epoch: {epoch} im_loss: {round(image_loss.item(), 2)} fn_loss: {rounc(final_loss.item(), 2)} | im_loss(avg): {np.mean(losses_image[-100:-1])} fn_loss(avg): {round(np.mean(losses_final[-100:-1]), 2)}")
        progress_bar.set_description(f"epoch: {epoch} im_loss: {round(image_loss.item(), 2)} fn_loss: {round(final_loss.item(), 2)}")
    
    losses_image_set.append(np.mean(losses_image))
    losses_final_set.append(np.mean(losses_final))
    results = pd.DataFrame({'imgage_loss(avg)': losses_image_set, 'fn_loss (avg)': losses_final_set})
    results.to_csv(f"{os.getcwd()}/result/train/{model_name}/results/train_metric{epoch}.csv", index = False)
    losses_image = []
    losses_final = []
...
#         progress_bar.set_description(f"epoch: {epoch} im_loss: {round(image_loss.item(), 2)} | im_loss(avg): {round(np.mean(losses_image[-100:-1]), 2)}")
#         # if i % 10000 == 0 or i == loader_len - 1:
#         # torch.save({
#         #       'image_model_state_dict': image_model.module.state_dict(),
#         #       'final_model_state_dict': final_model.module.state_dict(),
#         #       'image_optimizer_state_dict': image_optimizer.module.state_dict(),
#         #       'final_optimizer_state_dict': final_optimizer.module.state_dict(),
#         #       }, f'{os.getcwd()}/result/train/{model_name}/models/{epoch}.pth')
#         # torch.save({
#         #       'image_model_state_dict': image_model.state_dict(),
#         #       'final_model_state_dict': final_model.state_dict(),
#         #       'image_optimizer_state_dict': image_optimizer.state_dict(),
...
#         progress_bar.set_description(f"im_loss: {round(image_loss.item(), 2)} | im_loss(avg): {round(np.mean(losses_image), 2)}")
    
#     losses_image_set.append(np.mean(losses_image))
#     results = pd.DataFrame({'imgage_loss(avg)': losses_image_set})
#     results.to_csv(f"{os.getcwd()}/result/train/{model_name}/results/train_metric{gpu_device}_{epoch}.csv", index = False)
#     losses_image = []
